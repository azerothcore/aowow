    ...
    public function generate(array $ids = []) : bool
    {
        $ssQuery = '
            SELECT
                ...
                Targets,
                RequiresSpellFocus AS spellFocus,
                CastingTimeIndex,
                RecoveryTime AS recoveryTime, CategoryRecoveryTime AS recoveryTimeCategory,
                ProcChance,         ProcCharges,
                MaxLevel,           BaseLevel,          SpellLevel,
                DurationIndex,
                PowerType AS powerType,
                ManaCost AS powerCost,
                ManaCostPerLevel AS powerCostPerLevel,
                ManaPerSecond AS powerPerSecond,
                ManaPerSecondPerLevel AS powerPerSecondPerLevel,
                RangeIndex,
                CumulativeAura,
                Totem_1 AS tool1,   Totem_2 AS tool2,
                ...
                IF (Name_Lang_enUS IS NULL, "", CONCAT("Serverside - ", Name_Lang_enUS)) AS name_loc0,
                IF (Name_Lang_koKR IS NULL, "", CONCAT("Serverside - ", Name_Lang_koKR)) AS name_loc2,
                IF (Name_Lang_frFR IS NULL, "", CONCAT("Serverside - ", Name_Lang_frFR)) AS name_loc3,
                IF (Name_Lang_deDE IS NULL, "", CONCAT("Serverside - ", Name_Lang_deDE)) AS name_loc4,
                IF (Name_Lang_zhCN IS NULL, "", CONCAT("Serverside - ", Name_Lang_zhCN)) AS name_loc6,
                IF (Name_Lang_zhTW IS NULL, "", CONCAT("Serverside - ", Name_Lang_zhTW)) AS name_loc8,
                ...
                RequiredTotemCategoryID_1 AS toolCategory1, RequiredTotemCategoryID_2 AS toolCategory2,
                SchoolMask,
                RuneCostID AS runeCostId,
                PowerDisplayID AS powerDisplayId,
                EffectBonusMultiplier_1 AS effect1BonusMultiplier, EffectBonusMultiplier_2 AS effect2BonusMultiplier, EffectBonusMultiplier_3 AS effect3BonusMultiplier,
                SpellDescriptionVariableID AS spellDescriptionVariable,
                SpellDifficultyID AS spellDifficulty
            FROM
                spell_dbc
            WHERE
                id > ?d
